...
tf.flags.DEFINE_string('model_save_path', 'model/gan2/', 'directory of model file saved')
...
class DCGAN(object):
    ...
    def discriminator(self, x, is_train=True, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            out_1 = tf.layers.Conv2D(32, 4, 2, 'same',
            ...
            out_3 = tf.layers.Dense(100,
                                    kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out_2)
            out_3 = tf.layers.batch_normalization(out_3, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out_4 = tf.nn.leaky_relu(out_3)

            out_5 = tf.layers.Dense(1, activation='sigmoid',
                                    kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out_4)

            return out_5

    def generator(self, noise, is_train=True, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            out_1 = tf.layers.Dense(8 * IMAGE_SIZE * IMAGE_SIZE,
                                    kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(noise)
            out_1 = tf.reshape(out_1, [-1, IMAGE_SIZE // 2, IMAGE_SIZE // 2, 32])
            out_1 = tf.layers.batch_normalization(out_1, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out_1 = tf.nn.relu(out_1)

            out_2 = tf.layers.Conv2DTranspose(IMAGE_CHANNEL, 5, 2, 'same', activation='tanh',
                                              kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out_1)
