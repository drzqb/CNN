...
tf.flags.DEFINE_string('model_save_path', 'model/gan5/', 'directory of model file saved')
...
class CGAN(object):
    ...
    def generator(self, noise, c, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            out1 = tf.layers.dense(noise, 128,
                                   activation='relu',
                                   kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))
            out2 = tf.layers.dense(c, 128,
                                   activation='relu',
                                   kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))
            out = tf.layers.dense(tf.concat([out1, out2], axis=1), IMAGE_SIZE2,
                                  activation='sigmoid',
                                  kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))
    ...
    def discriminator(self, x, c, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            out = tf.layers.dense(tf.concat([x, c], axis=-1), 128,
                                  activation='relu',
                                  kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))
