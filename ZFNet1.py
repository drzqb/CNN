...
from tfrecord_utils import batched_data, single_example_parser
...
class User():
    ...
    def train():
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(1, CONFIG.epochs + 1):
                ...
                loss_, acc_, prediction_ = mylenet.train(sess, X[0], X[1])
                loss.append(loss_)
                acc.append(acc_)
                ...
                if epoch % CONFIG.per_save == 0:
                    acc_val = mylenet.eval(sess, X_val[0], X_val[1])
                    print(' acc_val: %.2f%%\n' % (100.0 * acc_val))
    ...
    def predict():
        ...
        with tf.Session() as sess:
            ...
            result = mylenet.predict(sess, X_val[0])
            print(result)
            print('----------------------------------------------------------------')
