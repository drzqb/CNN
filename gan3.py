'''
    DCGAN for beauty with more layers
'''
...
tf.flags.DEFINE_string('model_save_path', 'model/gan3/', 'directory of model file saved')
...
class DCGAN(object):
    ...
    def discriminator(self, x, is_train=True, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            out = tf.layers.Conv2D(8, 3, 2, 'same',
                                   kernel_initializer=tf.truncated_normal_initializer(stddev=STDDEV))(x)
            out = tf.nn.leaky_relu(out)

            out = tf.layers.Conv2D(16, 3, 2, 'same',
                                   kernel_initializer=tf.truncated_normal_initializer(stddev=STDDEV))(out)
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.leaky_relu(out)

            out = tf.layers.Conv2D(16, 4, 2, 'same',
                                   kernel_initializer=tf.truncated_normal_initializer(stddev=STDDEV))(out)
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.leaky_relu(out)

            out = tf.layers.Conv2D(32, 4, 2, 'same',
                                   kernel_initializer=tf.truncated_normal_initializer(stddev=STDDEV))(out)
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.leaky_relu(out)
    ...
    def generator(self, noise, is_train=True, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            out = tf.layers.Dense(IMAGE_SIZE * IMAGE_SIZE,
                                  kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(noise)
            out = tf.reshape(out, [-1, IMAGE_SIZE // 8, IMAGE_SIZE // 8, 64])
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.relu(out)

            out = tf.layers.Conv2DTranspose(32, 3, 2, 'same',
                                            kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out)
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.relu(out)

            out = tf.layers.Conv2DTranspose(8, 3, 2, 'same',
                                            kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out)
            out = tf.layers.batch_normalization(out, epsilon=EPS, momentum=MOMENTUM, training=is_train)
            out = tf.nn.relu(out)

            out = tf.layers.Conv2DTranspose(IMAGE_CHANNEL, 3, 2, 'same', activation='tanh',
                                            kernel_initializer=tf.random_normal_initializer(stddev=STDDEV))(out)
    ...
    def train_D(self, sess, X, Z):
        D_loss_curr, _ = sess.run([self.D_loss, self.D_train_op], feed_dict={self.X: X, self.Z: Z})

        return D_loss_curr

    def train_G(self, sess, Z):
        G_loss_curr, _ = sess.run([self.G_loss, self.G_train_op], feed_dict={self.Z: Z})

        return G_loss_curr

    def predict(self, sess, Z):
        result = sess.run(self.fake_image_pre, feed_dict={self.Z: Z})
        return result
